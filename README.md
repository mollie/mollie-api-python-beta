...
| --------- | ------ | ------------ | -------------------- |
| `api_key` | http   | HTTP Bearer  | `MOLLIE_API_KEY`     |
| `o_auth`  | oauth2 | OAuth2 token | `MOLLIE_O_AUTH`      |
...
import mollie_api_python_beta
from mollie_api_python_beta import Mollie
from mollie_api_python_beta.utils import BackoffStrategy, RetryConfig
import os
...
```python
import mollie_api_python_beta
from mollie_api_python_beta import Mollie, models
import os
...
s = Mollie(client=http_client)
```

or you could wrap the client with your own custom logic:
```python
from mollie_api_python_beta import Mollie
from mollie_api_python_beta.httpclient import AsyncHttpClient
import httpx
...
s = Mollie(async_client=CustomClient(httpx.AsyncClient()))
```
...
The `Mollie` class implements the context manager protocol and registers a finalizer function to close the underlying sync and async HTTPX clients it uses under the hood. This will close HTTP connections, release memory and free up other resources held by the SDK. In short-lived Python programs and notebooks that make a few SDK method calls, resource management may not be a concern. However, in longer-lived programs, it is beneficial to create a single SDK instance via a [context manager][context-manager] and reuse it across the application.
...
s = Mollie(debug_logger=logging.getLogger("mollie_api_python_beta"))
```
