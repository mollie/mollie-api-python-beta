...
from mollie_api_python_beta._hooks import HookContext
from mollie_api_python_beta.types import BaseModel, OptionalNullable, UNSET
from mollie_api_python_beta.utils import get_security_from_env
...
class PaymentLinks(BaseSDK):
    ...
    def update(
        ...
        *,
        ...
        :param allowed_methods: An array of payment methods that are allowed to be used for this payment link. When this parameter is not provided or is an empty array, all enabled payment methods will be available.  All methods supported by the Payments API are also supported here.  Possible values: `alma` `applepay` `bacs` `bancomatpay` `bancontact` `banktransfer` `belfius` `blik` `creditcard` `directdebit` `eps` `giftcard` `ideal` `kbc` `mybank` `paypal` `paysafecard` `pointofsale` `przelewy24` `satispay` `trustly` `twint`
        :param retries: Override the default retry configuration for this method
        :param server_url: Override the default server URL for this method
        :param timeout_ms: Override the default request timeout configuration for this method in milliseconds
        :param http_headers: Additional headers to set or replace on requests.
