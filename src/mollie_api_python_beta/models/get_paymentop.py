...
from mollie_api_python_beta import utils
from mollie_api_python_beta.types import (
    BaseModel,
    Nullable,
    OptionalNullable,
    UNSET,
...
from mollie_api_python_beta.utils import (
    FieldMetadata,
    PathParamMetadata,
    QueryParamMetadata,
)
...
class GetPaymentBillingAddressTypedDict(TypedDict):
    ...
    r"""Required for payment method `in3`."""
...
class GetPaymentResponseBodyTypedDict(TypedDict):
    ...
    Possible values: `alma` `applepay` `bacs` `bancomatpay` `bancontact` `banktransfer` `belfius` `blik` `creditcard` `directdebit` `eps` `giftcard` `ideal` `in3` `kbc` `mybank` `paypal` `paysafecard` `pointofsale` `przelewy24` `satispay` `trustly` `twint` `voucher`
    """
